...
NOTE: `restheart-mqtt` ships **separately** from RESTHeart and is not yet part of a release: every build is a snapshot of `master`. It is well covered — 395 unit tests, plus 24 integration tests that run the module inside a real RESTHeart against a Mosquitto broker and a MongoDB container on every change, including a database outage and an orderly shutdown — so it is solid enough to build on. What is not settled yet is the _surface_: configuration keys, the SSE envelope, the REST response and the shape of stored documents may still change between snapshots, so pin the build you install. Feedback now can still shape the design — https://github.com/SoftInstigate/restheart/issues[open an issue] with anything you find or miss.
