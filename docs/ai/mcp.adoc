...
The action left open says what it depends on, so the agent knows what to send: its note reads _whether this is permitted depends on the query parameter `ticket`, which the call carries…_. And when the rule that could apply writes fields into the body with `mergeRequest`, the action lists them as `server_sets` and its `body_schema` no longer requires them: the collection's schema describes the stored document, not what the caller sends.

The unfiltered catalog names each resource's actions with the parameters each one requires, so an agent can go from the listing to `call_api` without describing every resource first.
