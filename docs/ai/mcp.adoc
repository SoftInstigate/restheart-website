...
NOTE: changed in *RESTHeart v9.9*.
...
To decide what to announce, RESTHeart **reads the caller's own permissions** — it does not put the authorization chain a request invented for the occasion. For each MCP-enabled resource it asks whether some call could satisfy one of those permissions, and announces the resource as soon as the answer is yes.
...
**Announcing is not granting.** A resource in your catalog may still answer `403`, and a read is authorized when it happens, exactly as the equivalent REST `GET` would be.

==== Overriding what the catalog decides

Two keys of a resource's `mcp` block override the reading of the permissions. Both decide **disclosure**, never access.
