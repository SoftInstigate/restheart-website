...
A change stream is a live subscription, not a request with one response, so `call_api` does not execute it: an agent subscribes with `resources/subscribe` and receives `notifications/resources/updated` as events arrive. For code that opens the stream itself, `how_to_call` returns a `wss://` descriptor for the `websocket` transport and an `https://` one with `Accept: text/event-stream` for `sse`.
...
Change streams (they're a live subscription, not a request/response read — use `resources/subscribe`), databases (a container, not itself document-shaped), and GraphQL apps stay `list_apis`/`call_api`-only for now.
