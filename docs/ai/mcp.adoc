...
**Actions are never filtered.** A resource is described with every action its kind affords: a collection with all of them, an aggregation with its execution, a GraphQL app with its query. What a call may do is decided when it is made, and `call_api` says so: a `403` means the session's role cannot do that thing.
