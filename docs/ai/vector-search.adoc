...
The model is a property of the vector field, from 9.9: a collection may name its own `provider`, `model` and `dimensions` in the same block, and they win over the provider's configuration and over any `override-ai-*` the request carries. `voyage-law-2` on `/legal` and `voyage-code-4` on `/src`, with one key. `$vectorize` follows the same block, so a question is embedded with the model of the vectors it is searched against:
...
|`documentChunkingInterceptor` |Chunk GridFS uploads — code-aware (function/class boundaries) for recognized source file extensions, character-window otherwise. `chunk-size`, `chunk-overlap`, `target-collection`, `embedding-provider`.
