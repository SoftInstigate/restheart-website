...
*How.* `chunk-size` and `chunk-overlap` default to the interceptor's configuration. `splitter` chooses how the text is cut: `auto`, the default, at function and class boundaries for a recognized source file and in character windows otherwise; `text`, always in character windows, when the model embeds code better that way; `code`, at code boundaries whenever the language is recognized. Code gets no overlap unless the rule sets `chunk-overlap`.
