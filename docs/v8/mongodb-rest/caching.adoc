...
In the examples above, `cache` enables cursor caching.

If you add `count` to `GET /coll`, RESTHeart also computes count metadata for the response. This is handled separately from the cached page data: only the document page retrieval can hit the cache, while the count is still computed for each request.
