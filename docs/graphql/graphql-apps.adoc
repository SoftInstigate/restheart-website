...
applies_to: both
redirect_from:
  - /docs/graphql-websocket/graphql/graphql-apps
  - /docs/mongodb-graphql/graphql-apps
---
...
The descriptor is the header section of your GraphQL app definition, where you configure the application's identity and basic settings: its name (via `_id`), description, and the URI endpoint where it will be accessible.

NOTE: v9 has simplified the descriptor section to reduce configuration redundancy and make it more intuitive.

==== Required Fields

- `_id`: The document identifier, which also serves as the application name

==== Optional Fields

- `descriptor.description`: GraphQL application description
- `descriptor.enabled`: can be `true` or `false` (default: `true`)
- `descriptor.uri`: Custom endpoint path. If omitted, uses `/_id` as the URI

==== Simplified Configuration (v9)
...
This creates a GraphQL application accessible at `/graphql/myapp`.
...
This creates a GraphQL application accessible at `/graphql/v1/users`.

==== Key Changes from v8

RESTHeart v9 streamlines the GraphQL App Definition:

1. **Removed `descriptor.name`** - The `_id` field now serves as the sole identifier
2. **Optional URI** - When `descriptor.uri` is omitted, the URI auto-generates as `/{_id}`
3. **String `_id` required** - When not using a custom URI, `_id` must be a string
4. **URI collision prevention** - Validation prevents `_id` values from matching existing custom URIs and vice versa

**Migration from v8:**

If migrating from v8, remove the `descriptor.name` field from your GraphQL app definitions:
...
// v9 (new)
{
  "_id": "myapp",  // Use meaningful string IDs
  "descriptor": {
    "uri": "/myapp"  // Optional - will default to /myapp
  }
}
----

IMPORTANT: The `_id` field must be a string when relying on automatic URI generation. Custom URIs can use any `_id` format.
...
{
  "descriptor": "...",
  "schema": "type User{name: String surname: String email: String posts: [Post]} type Post{text: String author: User} type Query{users(limit: Int = 0, skip: Int = 0):[User]}",
  "mappings": "..."
}
----
...
Mappings can reference server-resolved variables through `$arg`, in both field-to-query and field-to-aggregation mappings. The value comes from the server, never from the client: a caller cannot supply it as a GraphQL argument or a query parameter.

[options="header"]
[cols="2,5"]
|===
|Variable |Resolves to
|`@user` |the authenticated account; which properties it has depends on how the caller authenticated — `@user._id` against MongoDB users, `@user.userid` against the file realm, `@user.sub` with a token. See link:/docs/security/permissions#identifying-the-caller[Identifying the caller]
|`@roles` |the authenticated user's roles, as an array
|`@authenticated` |`true` when the request is authenticated
|`@request` |the request; `@request.body`, `@request.method`, and nested properties with dot notation
|`@filter` |the request's `filter` query parameter
|`@qparams['name']` |one query parameter by name
|`@now` |the current timestamp
|`@rnd(bits)` |a cryptographically secure random hex string
|`@mongoPermissions` |the permissions applying to the request, e.g. `@mongoPermissions.readFilter`
|===

Any variable a plugin registers through the `VarResolver` SPI works here too — the Stripe module's `@subscription`, for instance.

NOTE: Before RESTHeart 9.9 only `@user` resolved inside a mapping; every other variable failed with `variable ... not bound`.

For example, filtering by the authenticated user:
...
The `$or` is there because the two kinds of caller are identified differently: `@user._id` for an account read from MongoDB users, `@user.sub` for one carried by a token. Each `$arg` takes `null` as its default so that the branch which does not apply simply does not match — without a default, a name the account does not carry fails the query with `variable not bound` (since 9.8.2; before, it bound `null`, and `{"_id": null}` matches any document without an `_id`, which no document has, so that particular case was harmless here).
...
=== Hiding fields by role: `@visible`

Write `@visible(roles: [...])` on a field to restrict who sees it. The directive is declared for you — do not declare it in your SDL:
...
type Employee {
  name: String
  department: String
  salary: Float @visible(roles: ["hr", "admin"])
}
----

A caller whose roles do not include `hr` or `admin` gets a schema without `salary`. Introspection does not list it, and asking for it is a validation error saying the field does not exist — so a caller who may not see a field does not learn it is there.

Undecorated fields are visible to everyone: visibility is opt-in per field, and adding the directive to a schema changes nothing until you write it on a field. An unauthenticated caller has no roles, so it sees only the fields no `@visible` restricts.

This also applies through MCP: the schema an agent introspects is the one its own roles can see.
