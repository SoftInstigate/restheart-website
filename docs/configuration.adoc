...
The following example uses the option `-e RHO="/core/name->'the-best-api-ever'"` to override the configuration parameters `/mclient/connection-string` and `/core/name`.
...
The following command overrides the property file in the container with the file `/host/path/to/restheart.yml` in the host machine.
...
Please note, that the Docker container still defines the following `RHO` variable which will override the paramenter in the configuration file:
