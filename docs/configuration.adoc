...
The configuration can still be overridden with the `RHO` env variable or passing an override file with the `-o` option.

[source,bash]
$ ROH='/core/name->"foo"' java -jar restheart.jar restheart.yml

[source,bash]
$ java -jar restheart.jar restheart.yml -o conf-overrides.conf
