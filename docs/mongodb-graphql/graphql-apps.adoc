...
graphql:
  ...
  app-def-cache-ttl: 10_000 # Not used anymore from v8.0.9, see https://github.com/SoftInstigate/restheart/issues/523
...
The GQL App Definitions caching is automatically managed by the system.

All existing GQL apps are cached at boot time.

Creating and updating an GQL app definition automatically updates the cache.

All cache entries are invalidated on any delete request involving the gql-apps collection.

Additionally, a new provider, `gql-app-definition-cache`, is introduced. This provider is available to other plugins, enabling them to extend the cache invalidation policy as needed.
