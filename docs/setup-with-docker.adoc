...
NOTE: for versions 6.0.x and 6.1.x, you also need to specify the option `-e RH_MONGO_URI='mongodb://host.docker.internal'` to override the default mongo uri. Starting from version 6.2.0, this is the default uri in the docker image and the override is no more needed.
