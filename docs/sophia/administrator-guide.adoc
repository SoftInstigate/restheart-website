...
For MCP clients, the agent is selected via the URL path (see link:/docs/sophia/mcp[Sophia MCP Server]).
...
. Fires a `RESPONSE_ASYNC` interceptor (`InviteEmailSender`) that sends a dark-themed HTML email via *AWS SES* (configured under `/emails` in `conf-prod.yml`).
...
. Sends a fresh invitation email via the same emails channel used for the first send.
...
See link:/docs/sophia/mcp[Sophia MCP Server] for full setup details and the OAuth alternative.
...
For first-time provisioning, AWS Bedrock IAM setup, and MongoDB Atlas vector index creation, see the link:/docs/sophia/setup[Setup Guide].
