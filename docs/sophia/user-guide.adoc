...
- The activation link is *single-use* and expires 7 days after it is sent. If it expires (or you lose the email), ask your administrator to *resend the invitation* — they can do it with one click from the *Admin > Users* page, which rotates the token and emails you a fresh link.
...
If you forget your password, click *Forgot your password?* on the login screen. Enter your email and Sophia sends a reset link to your inbox (valid for 1 hour). The page that opens lets you choose a new password — no policy re-acceptance needed.

For privacy reasons the response is intentionally neutral: you'll see "If an account exists, we've sent an email" regardless of whether the email is registered. This prevents attackers from probing the system to discover registered accounts. The same endpoint also rate-limits to one email per minute per address.

If you never completed your initial activation, the same forgot-password flow will instead resend the *invitation* email — Sophia tracks the distinction internally.
...
See link:/docs/sophia/mcp[Sophia MCP Server] for full setup instructions, including OAuth and API-token authentication for private agents.

== REST API

For programmatic access, Sophia exposes a full REST API for chat operations, document management, and semantic search.

See link:/docs/sophia/api-documentation[API Documentation] for endpoint reference and code samples.
...
- *Account* — your email (immutable), display name (you can edit it inline by clicking *Edit*), tenant, roles, status, and member-since date. If an administrator has set knowledge tag restrictions on your account, this section also shows them read-only — both the global default and any per-agent overrides — with a short note about how the filter applies to document retrieval.
...
You can edit your *display name* and your *password* from here. All other fields (roles, tenant, accessible agents, knowledge tags) are managed by an administrator — to change any of them, contact yours.
