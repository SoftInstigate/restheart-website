...
applies_to: both
redirect_from:
  - /docs/graphql-websocket/websocket/tutorial
  - /docs/mongodb-websocket/tutorial
---
...
Let's verify that our change streams were created successfully by checking the collection metadata with the `HAL` representation.

===== cURL

[source,bash]
----
curl -i -X GET '[RESTHEART-URL]/messages?rep=HAL' \
  -H 'Authorization: Basic [BASIC-AUTH]'
----
...
[source,bash]
----
http GET [RESTHEART-URL]/messages rep==HAL \
  'Authorization:Basic [BASIC-AUTH]'
----
...
[source,javascript]
----
fetch('[RESTHEART-URL]/messages?rep=HAL', {
  method: 'GET',
  headers: {
    'Authorization': 'Basic [BASIC-AUTH]'
  }
