...
This ensures that invariants declared on the users collection — for example that a `consents` field is always present — are enforced even for documents created or modified through `restheart-accounts` endpoints, which bypass the MongoService pipeline.
