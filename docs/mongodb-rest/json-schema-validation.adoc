...
* JSON Schema draft-04, draft-06 and draft-07, by each schema's `$schema`; draft-07 when it has none, since 9.9.0. Before 9.9.0 every schema was validated as draft-04, whatever its `$schema`. Draft 2019-09 and 2020-12 are refused with `400`
...
Since 9.9.0 the schema store's name is configurable, with `schema-store` in the `mongo` configuration; the default is `_schemas`, the name used on this page:
...
NOTE: RESTHeart Cloud uses `schemas`: a service's schema store is `/schemas`. With a RESTHeart Cloud service URL set in the examples configurator, the examples on this page use it.
...
NOTE: Starting from RESTHeart v9.7.0, the JSON Schema store is exposed as a `Provider` (`json-schemas`), making it accessible to any plugin — not just the MongoService pipeline.
...
[IMPORTANT]
====
The *implementation* still lives in `restheart-mongodb`, so the `json-schemas` provider only exists where that module is deployed.

If your plugin must keep working without `restheart-mongodb`, declare the injection as optional (available from version 9.8):
...
In earlier versions, resolve the provider optionally through the registry instead:
...
.Available from v9.7.0
NOTE: Starting from RESTHeart v9.7.0, the `restheart-accounts` plugin validates user documents against the collection's JSON Schema on creation and update.
...
This ensures that the rules declared on the users collection — for example that a `consents` field conforms to a specific structure when present — are enforced even for documents created or modified through `restheart-accounts` endpoints, which bypass the MongoService pipeline.
...
NOTE: Application-level fields such as `consents` should be declared optional in the schema. Their absence is enforced by guard rules at the application level, not by the schema. See link:https://cloud.restheart.com/docs/guards#_example_gating_on_consents[Gating on Consents] for the full pattern.

Validation is opt-in: no `jsonSchema` metadata on the collection means no validation, exactly as for any other collection.

See link:/docs/accounts/user-registration#_json_schema_validation_on_registration[User Registration — JSON Schema validation] for details and examples.

=== Bulk PATCH

A `PATCH` carrying update operators has nothing to validate until it has been applied, so it is checked after the write and undone if it fails. On a replica set that undo is the abort of the transaction the write runs in, and a bulk PATCH is validated like any other:
...
{
  "http status code": 400,
  "message": "Request content violates schema 'address': #/zip: expected type: String, found: Integer"
}
----

Not one document is written: either every matched document satisfies the schema or none of them is updated. The check reads the documents the patch touched — by id, so a document the update pushes out of the request's own `filter` does not escape it.

NOTE: Validated bulk PATCH is available from RESTHeart v9.9 and requires a replica set. On a standalone MongoDB it is refused — see <<Limitations>>.

The same mechanism is available to your own plugins: see link:/docs/framework/interceptors#_undoing_a_write_at_response_time_v9_9[Undoing a write at RESPONSE time].

=== Limitations

The jsonSchema validator has some limitations to be aware of:

* *Bulk PATCH on a standalone MongoDB*: it cannot be validated there, so it is refused. Without transactions there is no way to undo a bulk write.
...

{
  ...
  "message": "'jsonSchema' checker does not support bulk PATCH requests on a standalone MongoDB. Use a replica set, where the write runs in a transaction and is validated after it, or set 'skipNotSupported:true' to allow them unvalidated."
}
----

To let them through **unvalidated** instead, add the `skipNotSupported` metadata property. It has no effect on a replica set, where they are validated:
