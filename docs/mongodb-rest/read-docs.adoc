...
`?count=estimated` works on the collection list endpoint as well — `GET /<coll>?count=estimated` returns documents together with the metadata count, useful for paginated UIs that only need an approximate total.
