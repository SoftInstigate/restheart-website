...
Parsing rules for `count` are:

- `count`, `count=`, `count=true`, or any value other than `estimated` => count is enabled with exact strategy.
- `count=estimated` => count is enabled with estimated strategy, but only when `filter` is not present.
...
|===
| Request | Strategy | `X-Count-Strategy`

| `_size` | exact | `exact`
| `_size?count=estimated` | metadata count | `estimated`
| `_size?count=estimated&filter={...}` | exact (auto fallback) | `exact`
| `_size?filter={...}` | exact | `exact`
|===

`?count=estimated` works on the collection list endpoint as well — `GET /<coll>?count=estimated` returns documents together with the metadata count.
This remains estimated as long as no `filter` is provided. If `filter` is present, strategy is exact by design.
...
NOTE: The estimated count is read from collection metadata; it can drift slightly from the exact value after unclean shutdowns until the metadata is revalidated. Use it where speed matters more than precision.
