...
* Why and when to use multi-document transactions
* Understanding sessions and transaction lifecycle
* Creating and managing transaction sessions
...
* MongoDB v4.0 or later configured as a Replica Set
* RESTHeart running and connected to your MongoDB instance

TIP: A demo application showcasing transactions is available on GitHub at https://github.com/softInstigate/restheart-txn-demo
[.alert.alert-success]
****
*Stay consistent!*
...
|Write conflict |409 |Another transaction committed changes to the same documents. The error body carries `"retryable": true`
|Transaction expired |500 |Transaction exceeded the maximum runtime (default: 60 seconds)
|===

A write conflict is the one error a retry can clear: nothing was applied, and the same request sent again may well succeed. It is marked in the error body so the client does not have to guess:
...
Retry only on `retryable`. Other `409` responses — an ETag mismatch, a duplicate key, a violated link:/docs/mongodb-rest/constraints[data constraint] — repeat the same answer however many times they are sent. Back off between attempts and give up after a few.
