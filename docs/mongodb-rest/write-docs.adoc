...
This page covers everything you need to know about creating, updating, and deleting MongoDB data through RESTHeart's REST API.

=== What You'll Learn

* HTTP methods - Understanding POST, PUT, PATCH, and DELETE for different operations
* Creating documents - Insert single or multiple documents with auto-generated or custom IDs
* Updating documents - Modify specific fields (PATCH) vs replacing entire documents (PUT)
* MongoDB operators - Use $set, $inc, $push and other operators for advanced updates
* Bulk operations - Update or delete multiple documents matching a filter
* Write modes - Control upsert behavior and response formats

Start with the basics and work your way through, or jump to a specific section using the page navigation.
...
The URL is built for the client, not from what the node sees. Behind a proxy that terminates TLS the request reaches RESTHeart as plain `http`, so the scheme comes from `X-Forwarded-Proto` and the host from `X-Forwarded-Host`, each when present. Set `instance-base-url` in the `mongo` configuration to fix it instead — it includes the service's mount, e.g. `https://api.example.com/mongo`.

NOTE: From RESTHeart v9.9, `instance-base-url` can be overridden per request. A multi-tenant deployment, where one node serves several hosts, attaches the tenant's own value as `override-mongo-instance-base-url`, typically from an interceptor at `REQUEST_BEFORE_AUTH`:

[source,java]
----
req.attachParam("override-mongo-instance-base-url", "https://tenant1.example.com");
----
...
[WARNING]
====
**PUT replaces the entire document.** Any fields not included in your request will be deleted.

**PATCH updates only specified fields.** Other fields remain unchanged.

For most updates, use PATCH to avoid accidentally deleting fields.
====
...
[CAUTION]
====
Bulk operations affect **all documents matching the filter**. 

**Before running a bulk update or delete:**
1. Test your filter with a GET request to see which documents match
2. Consider using pagination to limit the impact
3. Have a backup or be able to restore data

Example: `curl "[RESTHEART-URL]/inventory/*?filter={\"status\":\"D\"}"` to verify which documents would be affected
====
...
To delete multiple files from a GridFS bucket in a single request, send `DELETE` on the bucket endpoint with a `filter` parameter:

[source,bash]
----
curl -i -X DELETE '[RESTHEART-URL]/mybucket.files/*' \
  -G --data-urlencode 'filter={"metadata.project":"old-project"}' \
  -H "Authorization: Basic [BASIC-AUTH]"
----

RESTHeart collects all matching file IDs first, then deletes each one. Individual failures are logged as warnings but do not abort the operation.

[IMPORTANT]
====
Omitting `filter` deletes **all files** in the bucket. Always verify your filter with a `GET` first.
====

Requires `allowBulkDelete: true` in the ACL permission for the bucket path.
