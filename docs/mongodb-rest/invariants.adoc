...
A collection can declare rules that span documents, and the server enforces them on every write. `jsonSchema` validates one document in isolation; an invariant validates the collection: no balance goes negative, at least one admin remains, no booking overlaps another.

An invariant is a property of the data, not of the caller. It applies whatever permission authorized the write, whatever the role, `admin` included.
...
**A replica set is required.** Without transactions a write that breaks a rule could not be undone, so a write to a collection declaring invariants is refused with `501` rather than let through unchecked. Invariants live in collection metadata, so this cannot be reported at startup — it is reported on the write.
...
**Pipelines are held to `aggregationSecurity`**, unchanged and in full — the same settings that constrain `aggrs`. `$lookup`, `$merge`, `$out`, `$graphLookup` and `$unionWith` are blacklisted by default, so an invariant reads only its own collection unless the deployment has already decided otherwise for its aggregations.
