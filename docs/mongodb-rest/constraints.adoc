...
applies_to: both
redirect_from:
  - /docs/mongodb-rest/constraints
---

A collection can declare rules that span documents, and the server enforces them on every write. `jsonSchema` validates one document in isolation; a constraint validates the collection: no balance goes negative, at least one admin remains, no booking overlaps another.
