...
Retry with a backoff, and give up after a few attempts — a collection under contention serializes its writes, so retrying forever only makes the queue longer.
...
`retryable` is not specific to constraints: any write in a transaction that aborts on a transient conflict carries it. Every other `409` — a violated constraint, an ETag mismatch, a duplicate key — does not.
