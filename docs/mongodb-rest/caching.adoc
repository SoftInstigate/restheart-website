...
MongoDB link:https://docs.mongodb.org/manual/reference/method/cursor.skip/#cursor.skip[cursor.skip()] method **that slows down linearly**.
...
- `cache` enables cursor caching (the actual topic of this page).

Use `count` only when the client needs documents and total size together (for example, paginated UIs).
If you only need the count, prefer `GET /coll/_size`.
For large collections where an approximate total is acceptable, use `count=estimated` and verify `X-Count-Strategy: estimated`. If `filter` is present, strategy falls back to exact by design (see link:/docs/mongodb-rest/read-docs#estimated-counts-fast-counting-on-large-collections[Estimated Counts]).
