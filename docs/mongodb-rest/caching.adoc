...
NOTE: On the `/_size` endpoint, counting is already implied, so adding `?count` is redundant and behaves like plain `/_size` (exact count).

For count strategy details (including `count=estimated` and automatic fallback with filters), see link:/docs/mongodb-rest/read-docs#estimated-counts-fast-counting-on-large-collections[Estimated Counts].
