...
[source,bash]
----
curl -i -X PATCH "[INSTANCE-URL]/mycollection?wm=upsert" \
  -u "[BASIC-AUTH]" \
...
}' | http PATCH "[INSTANCE-URL]/mycollection?wm=upsert" \
...
[source,javascript]
----
fetch('[INSTANCE-URL]/mycollection?wm=upsert', {
  method: 'PATCH',
  headers: {
    'Authorization': 'Basic [BASIC-AUTH]',
    'Content-Type': 'application/json'
...
[source,bash]
----
curl -i -X GET "[INSTANCE-URL]/mycollection/_aggrs/paginated-results?page=3&pagesize=25" \
  -u "[BASIC-AUTH]"
----
...
[source,bash]
----
curl -i -X GET "[INSTANCE-URL]/mycollection/_aggrs/sales-summary" \
  -u "[BASIC-AUTH]"
----
...
[source,bash]
----
curl -i -X GET "[INSTANCE-URL]/categorySalesSummary" \
  -u "[BASIC-AUTH]"
----
...
[source,bash]
----
curl -i -X GET "[INSTANCE-URL]/mycollection/_aggrs/my-pipeline?sid=session-id&txn=transaction-id" \
  -u "[BASIC-AUTH]"
----
