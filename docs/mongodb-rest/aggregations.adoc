...
|`?status=A`
|the string `"A"` (not valid JSON on its own, so taken literally — no need to quote it)
...
|`?opts={"limit":10,"skip":20}`
|the object `{"limit": 10, "skip": 20}`

|`?date={"$date":1700000000000}`
|the BSON date it represents — MongoDB Extended JSON types (`$date`, `$oid`, `$numberLong`, ...) work too
|===

NOTE: This shorthand works the same way for xref:/docs/websocket/variables.adoc[change stream variables], since both are bound through the same mechanism.
...
|`@user` |The authenticated account. Which properties it carries depends on how the caller authenticated — `@user._id` against MongoDB users, `@user.userid` against the file realm, `@user.sub` with a token. A name the account does not carry fails the request with `variable not bound` unless the `$var` declares a default, and a value supplied by the caller for a `@` name is ignored. See link:/docs/security/permissions#identifying-the-caller[Identifying the caller]
...
IMPORTANT: These security controls are enabled by default with conservative settings to protect your data
