...
A different approach that can help you extending the API us using the link:/docs/proxy[proxying] feature.

You can add a side microservice, for instance using NodeJs, that implements a Web Service. This microservice can execute requests to RESTHeart if it needs to interact with RESTHeart, for instance to to read or write data to MongoDB via the REST API.
