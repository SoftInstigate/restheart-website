...
This guide explains how to configure RESTHeart for your environment. RESTHeart offers flexible configuration options, from using sensible defaults to fine-tuning every aspect of its behavior.

==== What You'll Learn

. How to run RESTHeart with the default configuration
. How to generate and use a configuration file
. How to override configuration options using the `RHO` environment variable
. How to use configuration override files
. Understanding configuration syntax and validation

TIP: For most deployments, the default configuration with a few `RHO` overrides is sufficient. Only create a full configuration file if you need extensive customization.
