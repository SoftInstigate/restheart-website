...
That is why raising a price later never means editing `restheart.yml`: `GET /stripe/plans` reads name, description and amount from Stripe at request time.
====
...
[WARNING]
====
Never commit this key, and never paste it into `restheart.yml`.
...
stripeConfig:
  ...
  success-url:       "http://localhost:4200/billing?success=true"
  cancel-url:        "http://localhost:4200/billing?canceled=true"
  portal-return-url: "http://localhost:4200/billing"

  plans:
    free:
      seats: { mode: capped, max: 1 }
    gold:
      price-id-monthly: price_1AbCdEfGhIjKlMnOpQrStUvW   # <- yours
      price-id-annual:  price_1PqRsTuVwXyZaBcDeFgHiJkL   # <- yours
...
Restart RESTHeart and check the startup log:
...
[TIP]
====
If `gold` comes back without `prices`, the price ids in `restheart.yml` do not exist in this Stripe account.
...
| 1
| Recreate the products and prices in *live mode* — test-mode price ids do not exist in live mode. Update `restheart.yml` with the live ids.
...
| 7
| Configure the Customer Portal in *Settings → Billing → Customer portal* — which plan changes and cancellation options customers get is decided there, not in `restheart.yml`.
