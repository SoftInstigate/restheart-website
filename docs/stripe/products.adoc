...
[TIP]
====
`@restheart-cloud/kit` does both for you: `readOrderRef()` / `clearOrderRef()` for the reference, and `waitForOrder()` for the poll, which rejects with a distinct `WaitTimeoutError` so a late webhook never renders as a failed payment. See the https://github.com/SoftInstigate/restheart-cloud-kit/tree/main/packages/kit#payments[kit's payments guide].
====
