...
| `override-stripe-products`
| `stripeConfig.products`
| Replaces the *entire* products configuration — same wholesale-replacement contract as `override-stripe-plans`, not a field-by-field override. Absent means products mode is off for this tenant.

| `override-stripe-tmpl-{name}`
| built-in / configured template
| Inline HTML for one notification template — subscription and order notifications alike, see below

| `override-stripe-notify-{name}-enabled`
| `stripeConfig.subscriptions.notifications.{name}.enabled`
| Enables or disables one *subscription* notification. Order notifications have no equivalent: their enabled flag travels inside `override-stripe-products` itself, in `products.notifications.{name}.enabled`
...
`{name}` in `override-stripe-tmpl-{name}` is one of the subscription notifications
(`payment-failed`, `trial-will-end`, `subscription-canceled`, `over-limit`) or the order
notifications (`order-confirmed`, `order-refunded`) — the parameter is generic on the name, so
both sets are attached the same way and neither collides with the other.
