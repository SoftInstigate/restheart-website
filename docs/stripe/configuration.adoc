...
stripeConfig:
  ...
  products:
    ...
    success-url:        # checkout redirect — supports {ORDER_ID} / {ORDER_SECRET}
    cancel-url:         # checkout redirect
...
| `success-url`
| _(empty)_
| Checkout redirect after payment. Supports `+{ORDER_ID}+` and `+{ORDER_SECRET}+`, interpolated when the session is created — put the secret in the fragment. See xref:products.adoc#_the_return_page[The return page].
