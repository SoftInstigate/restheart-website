...
|**jsonSchema**|Checker|Checks the request according to the specified JSON schema. More info at [JSON Schema Validation](/docs/v4/json-schema-validation/)|`{ "checkers": [ { "name": "jsonSchema", "args": { "schemaId": "inventory", "schemaStoreDb": "restheart" } } ] }`|
