...
`$unauthenticated` is the same name `mongoAclAuthorizer` uses for a request with no account, so
permissions and predicates speak of anonymity in one vocabulary rather than two.
...
rule written for guests must not apply to it.
...
|`@roles`
|The caller's roles, as an array — `["$unauthenticated"]` when nobody is signed in. Available from RESTHeart 9.8.
|`@request`
