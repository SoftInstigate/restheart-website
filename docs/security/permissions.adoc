...
**Failure semantics.** Returning `null` (or throwing) from `resolve()` is treated as "unresolved": in a `readFilter`/`mergeRequest` document the value becomes MongoDB `null`; in a `predicate` string the expression is replaced with an unguessable random token, so any comparison against it fails. A broken resolver denies access, it never widens it.
...
NOTE: Missing properties and non-JSON/BSON requests resolve to `null`, implementing fail-safe denial semantics.
