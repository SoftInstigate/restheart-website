...
==== Denial reason in the response body (since 9.9)

By default a request vetoed by `OriginVetoer` gets a plain `403 Forbidden`
with an empty body. Since 9.9.0, `OriginVetoer` attaches a specific reason
for the denial, which RESTHeart returns as the `message` field of a JSON
response body:
...
or, when the `Origin` header itself is missing and `allow-missing-origin` is
`false`:
...
See xref:../framework/authorizers.adoc#_explaining_a_denial[Explaining a denial]
for how this mechanism works and how to use it in a custom `Authorizer`.
