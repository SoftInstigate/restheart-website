...
. Configure Access Control Lists (ACLs) for fine-grained permissions
. Set up authentication to verify user identities
. Implement authorization to control what users can access
. Test security configurations to ensure they work correctly

By the end, you'll have a solid foundation in securing RESTHeart applications with proper authentication and authorization.
...
TIP: If you're using RESTHeart Cloud, follow the link:https://cloud.restheart.com/docs/root-user-setup[Root User Setup] guide to create your root user with the `root` role and full permissions.
...
NOTE: The permissions below identify the caller by `@user._id`, which is right for this tutorial: users are defined in MongoDB and authenticate with Basic auth. A caller authenticated with a **token** has no `_id` — a JWT account carries the token's claims — and is identified by `@user.sub` instead. Naming a property the account does not carry leaves the variable unresolved: since 9.8.2 the filter then matches no document and a `mergeRequest` refuses the write with `403`; before 9.8.2 it became `null`, which matched every document without that field. See link:/docs/security/permissions#identifying-the-caller[Identifying the caller].
...
- link:/docs/security/authentication/overview[Authentication] - Different authentication mechanisms (JWT, OAuth2, etc.)
