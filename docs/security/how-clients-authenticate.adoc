...
NOTE: The `restheart-accounts` auto-login endpoints (`/auth/verify`, `/auth/activate`, `/auth/reset-password`, `/auth/switch-team`, and the OAuth callback) expose the same choice through a `delivery` query parameter — `cookie`, `body` (since 9.6.0), or `fragment` — mapping onto the three mechanisms above. See link:/docs/accounts/auth-flows[Authentication Flows].
