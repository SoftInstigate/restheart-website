...
An OAuth client renews the same token with `POST /token`, `grant_type=refresh_token` — see xref:/docs/security/oauth.adoc#_token_renewal[Token Renewal]. That form also accepts a token that expired within `authTokenService/refresh-grace-seconds`; `?renew` requires a valid one.
...
Two claims are always present and describe the *session* rather than the token, which `iat` and `exp` already do:
...
| `auth_time`
| When the user actually authenticated, in epoch seconds. Unchanged across renewals, so it survives a chain of them.

| `renewals`
| `0` on a token that follows an authentication, `+1` on each renewal.
|===

Both are readable in ACL rules as `@user.auth_time` and `@user.renewals`, which is how a deployment bounds the renewal chain without a plugin — see xref:/docs/security/oauth.adoc#_bounding_the_renewal_chain[Bounding the renewal chain]. Renewing with username and password starts a new chain rather than continuing one.

CAUTION: Both claims are available since RESTHeart 9.9.0.
