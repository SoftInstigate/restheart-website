...
|Define role-based GraphQL applications
|Mandatory
|===
...
The GraphQL API is read-only, so you should only pay attention to avoid it to expose sensitive information to users.

NOTE: This very important due to the nature of GraphQL that allows the client to request any data, in any format allowed by the GraphQL schema and related queries.
