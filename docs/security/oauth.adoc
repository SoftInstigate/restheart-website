...
. the person enters username and password; the page asks `GET /authorize/offer` with them, as `Basic` credentials and the same query string it received;
...
. the person chooses and the page posts to `/authorize` as in Step 2, credentials in the form body and the choices in the query string, one `choice.<name>` parameter per choice.
