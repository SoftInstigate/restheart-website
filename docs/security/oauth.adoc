...
----
authTokenService:
  refresh-grace-seconds: 60
----

Only renewal accepts an expired token; it opens and reads nothing. Sixty seconds covers the time a client takes to notice a `401` and act on it, plus clock skew. Raising it to a token lifetime is equivalent to raising `ttl` by the same amount, for anyone holding the token — a renewal yields a token that reads.
...
----
authTokenService:
  refresh-grace-seconds: 60   # how long past expiry a token may still be renewed
----
