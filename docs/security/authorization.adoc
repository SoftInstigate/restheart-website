...
|`@user`
|the account (excluding the password). What it carries depends on the authenticator: `@user._id` with `MongoRealmAuthenticator`, `@user.userid` with `FileRealmAuthenticator`, `@user.sub` for a JWT
|`@authenticated`
|`'true'` when the request carries an account, `'false'` when it does not
|`@roles`
|the caller's roles, as an array
|`@request`
|the properties of the request, e.g. `@request.remoteIp`
|`@request.body`
|the request body, e.g. `@request.body.amount`
|`@qparams['key']`
|a query parameter's value, e.g. `@qparams['status']`
|`@rnd(bits)`
...
WARNING: **A JWT account has no `_id`**, so `{"owner": "@user._id"}` leaves the variable unresolved for every caller authenticated with a token. Since 9.8.2 an unresolved variable in a filter matches no document, and refuses a `mergeRequest` write with `403`; before 9.8.2 it became `null`, which matches every document that has no owner. Identify a JWT caller by `@user.sub`. See link:/docs/security/permissions#identifying-the-caller[Identifying the caller] for the full list and for permissions that must serve both kinds of caller.
...
=== Saying why a request was denied

A `403` is normally empty, which makes a denial hard to diagnose — especially on a deployment where you cannot read the server logs. An authorizer can attach a reason, and RESTHeart returns it as the response body:
...
`originVetoer` already does this, so a request blocked by CSRF protection says so instead of failing silently.

Attach a reason only when it tells the caller something they are entitled to know. A message naming which collection or which role was missing turns a `403` into information about a resource the caller could not otherwise see.
