...
Each public-facing service registers its own ACL allow rule automatically on startup via the `ACLRegistry`. No manual ACL entry is needed for these paths. If a service is disabled, its path is not opened.
