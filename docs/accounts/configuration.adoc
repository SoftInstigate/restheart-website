...
| `override-accounts-oauth-google-client-id` | `TeamConfigInterceptor` | `oauthConfig.providers.google.client-id`
| `override-accounts-oauth-google-client-secret` | `TeamConfigInterceptor` | `oauthConfig.providers.google.client-secret`
| `override-accounts-users-unrestricted-roles` | `TeamConfigInterceptor` | `accountsConfig.users-unrestricted-roles` (since 9.6.0)

|===

NOTE: `override-accounts-users-unrestricted-roles` controls the link:/docs/accounts/security#users-self-service-write-restriction[`/users` self-service write restriction] on a per-team basis — see that section for the full explanation of what it exempts and why.
