...
`listTeamMembers`, `createTeam`, `updateTeam`, and `deleteTeam` (and the `TeamMember` record) were added in *9.6.0*.
The interface and its types (`Membership`, `TeamRef`, `TeamMember`) are defined in *`restheart-commons`* (`org.restheart.plugins.accounts`), so custom implementations depend only on `restheart-commons` — not on `restheart-accounts`.
====

By default, `restheart-accounts` stores team membership directly on the user document using the `team` / `teams` schema.
...
| `POST /auth/switch-team`, `POST /auth/accept-invite` (switches active org on acceptance)
...
All SPI methods that receive or return a team identifier use `org.bson.BsonValue` rather than `String`.
...
At HTTP boundaries (request bodies, response bodies), team IDs are serialized as *MongoDB extended JSON*:

* An ObjectId team appears as `{"$oid": "64a1b2c3…"}` in `GET /auth/teams` response items.
* The `POST /auth/switch-team` response and the `team` JWT claim carry the active team as `{ _id, role }` — e.g. `{ "_id": {"$oid": "..."}, "role": "owner" }`.
* Clients must send the same format in the `POST /auth/switch-team` request body.
...
accountsConfig:
  ...
  # Default: "team" if your domain uses organisations.
  ...
  #   GET    /auth/teams          POST   /auth/teams
  #   POST   /auth/switch-team
  #   GET    /auth/team/members
  #   PATCH  /auth/team           DELETE /auth/team
...
accountsConfig:
  db: myapp
  team-claim-name: org          # JWT carries "org" instead of "team"
...
Before `activateViaOAuth()` is called, the OAuth callback resolves the pending invitation from `auth_invitations` using the `pendingInviteToken` stored in the CSRF state, and calls `addMember(userId, teamId, role) to set the user's team.
...
public class OrgMembershipProvider implements MembershipProvider {
    ...
    @Override
    public TeamRef createInitialTeam(String userId, String orgName) {
        ...
        return new TeamRef(orgId, orgName);   // BsonObjectId, not String
    }
...
public interface MembershipProvider {
     ...
     * The returned TeamRef is embedded as the JWT team claim.
     */
    TeamRef createInitialTeam(String userId, String teamName);

    /**
     * Returns true if the user is already a member of the given team.
     * Used by /auth/invite to prevent duplicate memberships.
     */
    boolean isMember(String userId, BsonValue teamId);

    /**
     * Adds a user to a team with the specified role (idempotent).
     * If the user has no active team yet, this should also set it.
     */
     ...
     * Used by GET /auth/teams.
     */
    List<Membership> listMemberships(String userId);

    /**
     * Sets the given team as the user's active membership.
     * Used by POST /auth/switch-team.
     ...
     * If the team was the user's active team, the active-team field must be cleared.
    ...
    /**
     * Updates the org-level role of a member within the given team.
     ...
     * from each member's user document. Called by GET /auth/team/members.
     * Default: throws UnsupportedOperationException.
     */
    ...
    /**
     * Creates an additional team for an already-registered user, assigns them
     * the owner role, and sets it as their newly active membership. Unlike
     * createInitialTeam, this may be called any number of times for the same
     * user. Called by POST /auth/teams.
     * Default: throws UnsupportedOperationException.
     */
    ...
    /**
     * Updates a team's own fields (name and/or description, partial update —
     * either parameter may be null to leave that field unchanged).
     * Called by PATCH /auth/team.
     * Default: throws UnsupportedOperationException.
     */
    ...
    /**
     * Deletes a team, but only if it has no other members — implementations
     * must enforce this atomically, server-side (a client-side pre-check is a
     * race condition against a concurrent invite acceptance). On success, also
     * clears the caller's own membership entry and active-team pointer.
     * Called by DELETE /auth/team.
     * Returns false (does not throw) if the team still has other members, or
     * no longer exists.
     * Default: throws UnsupportedOperationException.
     */
...
// Tenant reference returned by createInitialTeam/createTeam — id is the native BsonValue
public record TeamRef(BsonValue id, String displayName) {}

// Snapshot of a single membership — teamId is the native BsonValue
public record Membership(BsonValue teamId, String displayName, String role, boolean active) {}

// A team member with display info denormalized from their user document,
// returned by listTeamMembers — email is the member's identifier
public record TeamMember(String email, String name, String role, Instant joinedAt) {}
----

=== Built-in default provider

`DefaultMembershipProvider` preserves the `team` / `teams` schema and stores team identifiers as MongoDB `ObjectId` values.
...
// users collection — schema managed by DefaultMembershipProvider
{
  ...
  "team":  { "$oid": "64a1b2c3d4e5f6a7b8c9d0e1" },   // active team (ObjectId)
  "teams": [
...
| `createInitialTeam`
| Inserts a `teams` document with `ObjectId` `_id`; `$set team` (ObjectId), `$addToSet teams` on the user

| `isMember`
| Reads `team` + `teams` on the user document; compares by native `BsonValue.equals()`

| `addMember`
| `$addToSet teams`; sets `team` via `$set` only if the field is absent

| `activeMembership`
| Reads the `team` field as-is (`BsonValue`); resolves team name from the `teams` collection

| `listMemberships`
| Reads the `teams` array as-is; resolves team names from the `teams` collection

| `setActiveMembership`
| `$set team` on the user document with `{ _id: <BsonValue>, role: <String> }`

| `removeMember`
| `$pull` from `user.teams[]` and `team.members[]`; if the removed team was active, `$unset team` on the user

| `updateMemberRole`
| Positional `$set teams.$.role` on the user; positional `$set members.$.role` on the team

| `listTeamMembers`
| Reads `team.members[]`; batch-fetches (`$in`) the corresponding `users` documents to denormalize display names from `profile.name` / `profile.surname`

| `createTeam`
| Same as `createInitialTeam`, but unconditionally `$set team` (vs. only-if-absent) so the new team becomes active immediately

| `updateTeam`
| Partial `$set` of `name` / `description` on the `teams` document

| `deleteTeam`
| Atomic `findOneAndDelete` on the `teams` document guarded by `$expr: {$size: "$members"} <= 1`; on success, `$pull` from `user.teams[]` and `$unset team` if it was active
|===
...
* `team-claim-name` defaults to `"team"` — JWT payload is unchanged.
...
[NOTE]
====
If your existing user documents store `team` / `teams.id` as plain hex strings (the 9.4 format), they continue to work: `activeMembership` and `listMemberships` read the stored value as-is via `BsonValue`, so a stored `BsonString("64a1…")` is returned without conversion.
