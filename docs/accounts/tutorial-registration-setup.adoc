...
`restheart-accounts` uses the **emails** module for SMTP delivery, injected via the `EmailSender` interface (`restheart-commons`).
...
[source,bash]
----
export RHO="/emails/smtp-password->'${SMTP_PASSWORD}'"
...
<2> Appears in email subjects and bodies as `{{app-name}}`.
...
Each `restheart-accounts` service registers its own ACL allow rule automatically on startup via the `ACLRegistry`. No manual ACL entry is needed for the public auth endpoints. If a service is disabled, its path is not opened.

Add rules only for the protected endpoints (invite management) and your application's own paths:
...
fileAclAuthorizer:
  ...
  permissions:
    ...
    - role: user
      ...
      priority: 10
      mongo:
        readFilter: '{"team": "@user.team._id"}'
        writeFilter: '{"team": "@user.team._id"}'
        mergeRequest:
          team: "@user.team._id"
----
...
| `{{app-name}}`   | value of `accountsConfig.app-name`
| `{{first-name}}` | recipient's first name (from `profile.name`)
...
| `verification.html`  | `{{verification-url}}`
| `password-reset.html`| `{{reset-url}}`
| `invite.html`        | `{{inviter-name}}`, `{{team-name}}`, `{{invite-url}}`
|===
...
New Google users are created with `roles: ["user"]` (the configured `default-role`; Google already verified the email)
...
A minimal, working configuration for a single-team app:
...
* [ ] ACL rules for invite management endpoints (`owner` role) — public endpoints are handled automatically by each service
* [ ] `mongoRealmAuthenticator.users-db` and `users-collection` set — accounts sources both from here
