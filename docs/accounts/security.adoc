...
Claims are read from the properties of the account that authenticated the request, not from a fresh database read. What that means depends on how the caller authenticated:
...
* with an existing JWT, the properties are that token's own claims — a field that was not in it cannot appear in a token derived from it, not even with `GET /token?renew=true`

So after adding a claim to the list, or after writing a new field to a user document, an existing token must be replaced by one issued from credentials for the change to show up.
