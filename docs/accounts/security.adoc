...
`accountsConfig.account-properties-claims` (and, in a multi-tenant deployment, its per-team override `override-accounts-account-properties-claims`) selects which fields of the user document are copied into the JWT issued by accounts endpoints (`/auth/verify`, `/auth/activate`, `/auth/reset-password`, `/auth/switch-team`, `GET`/`POST /auth/teams`, the OAuth callback).

A JWT payload is base64, not encrypted — the client reads every claim it carries. Listing a sensitive field here would ship it to the browser on every request. This matters more once the list is a per-tenant override: a tenant now holds a lever that used to belong only to the node operator.

`JwtHelper` enforces a fixed denylist at token issuance, regardless of whether the list came from static configuration or from the per-tenant override:
...
These are one-shot credentials or the password hash, never eligible to become a JWT claim. The filter is applied at issuance rather than at configuration validation, so it holds regardless of which layer wrote the list — static YAML, a `TeamConfigInterceptor` override, or a future custom source.
