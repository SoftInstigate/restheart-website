...
2. User does not exist: create user with `roles: ["$unauthenticated"]` (no `inviteToken` on user document).
3. Call `MembershipProvider.addMember()` immediately so `user.tenant` is set (required for JWT issuance on activation).
...
5. Delete invitation from `auth_invitations` (one-shot token).
6. Issue JWT with active tenant (set at invite time via `addMember()`).
7. Return `200 OK` + JWT cookie (auto-login).
...
sequenceDiagram
  ...
  R->>P: delete auth_invitations doc
...
5. Delete the invitation document from `auth_invitations`.
6. Return `200 OK`.
