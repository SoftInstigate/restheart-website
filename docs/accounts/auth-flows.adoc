...
2. Store `inviteToken`, `inviteOrgId`, `inviteRole` on the user document.
...
1. Find user by authenticated email.
2. Verify invite token matches and is not expired (7 days).
3. `MembershipProvider.addMember(userId, orgId, role)` — adds user to team.
4. Clear invite fields (`inviteToken`, `inviteCreatedAt`, `inviteOrgId`, `inviteRole`).
5. Return `200 OK`.
