...
. **If the user exists but is NOT in this tenant** → create an `auth_invitations` document with `isNewUser: false`. Do NOT add membership yet. Send invite email with link to `{frontend-url}/invitations/accept?email=...&token=...`.
. Return `201 Created`.
...
. `MembershipProvider.addMember(userId, orgId, role)` — adds user to the org and sets active tenant (reads `orgId` and `role` from the invitation document).
...
. Find invitation by `(email, pendingInviteToken)` in `auth_invitations`; call `MembershipProvider.addMember(userId, orgId, role)` to set tenant.
