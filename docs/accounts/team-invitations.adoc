...
. **If the user does not exist** → create user with `roles: ["$unauthenticated"]`, call `MembershipProvider.addMember()` immediately (sets `user.tenant` for JWT issuance at activation time). Create an `auth_invitations` document with `isNewUser: true`. Send activation email with link to `{frontend-url}/auth/activate?email=...&token=...`.
...
. Issue JWT + cookie → auto-login (tenant already set by `addMember()` at invite time).
. Return `200 OK`.
