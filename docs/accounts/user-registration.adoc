...
Application-level fields such as `consents` should be declared optional in the schema — their absence is enforced by guard rules, not by the schema. The schema validates the _format_ when the field is present. See link:https://cloud.restheart.com/docs/guards#_example_gating_on_consents[Gating on Consents] for the full pattern.
