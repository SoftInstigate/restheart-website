...
. The collection properties are read from `_properties._properties.{usersCollection}` in the effective database. The collection name comes from `accountsConfig/users-collection`, which must match `mongoRealmAuthenticator/users-collection`.
