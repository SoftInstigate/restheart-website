...
| `GET`   | `/auth/tenants`                | authenticated | List the caller's team memberships †
| `POST`  | `/auth/switch-tenant`          | authenticated | Switch active team; reissues JWT cookie †
...
See xref:multi-tenancy.adoc[Multi-tenancy] for the per-request override mechanism and xref:membership-providers.adoc[Custom Membership Providers] for replacing the built-in team/tenant storage model.
