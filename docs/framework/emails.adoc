...
The plugin is used by `restheart-accounts` for transactional emails (registration verification, team invitations, password reset), by the link:/docs/deployment/monitoring#workers-watchdog[workers watchdog] to mail its alarm, and can be used by any custom plugin that needs to send emails.
...
Code that has to mail *because* requests are not being served must not use `sendEmailAsync()`: it queues the send on the executor that is stuck. The workers watchdog calls `sendEmail()` from a platform thread of its own for this reason.
