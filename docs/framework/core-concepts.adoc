...
TIP: Use `@Inject(value = "provider", required = false)` for optional dependencies. If the provider is missing, the field is `null` and the plugin stays enabled. Available from version 9.8 — see link:/docs/framework/providers#_optional_dependencies[Optional Dependencies].
