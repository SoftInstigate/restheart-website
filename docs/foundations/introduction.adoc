...
RESTHeart is a modern Java framework — built on Java 25, virtual threads, and GraalVM native images — that turns a MongoDB database into a REST, GraphQL, WebSocket, and SSE API, with authentication, authorization, and real-time change streams already wired in.

Point it at a MongoDB instance and the API is there: no routes to write, no permission checks to hand-code, no pagination or filtering logic to duplicate across endpoints. 

Permissions and behavior are configured declaratively. Custom logic goes into plugins, written in Java, Kotlin, JavaScript, or TypeScript, only for what a data API cannot express.

RESTHeart has two faces, and understanding this distinction is crucial:
...
TIP: See it for yourself with a link:/#starter-apps-content[starter app]: sign-up, login, teams and data, with *zero* lines of backend code — clone it, point it at a free RESTHeart Cloud service, run it.
