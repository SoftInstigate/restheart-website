...
TIP: Use `@Inject(value = "myProvider", required = false)` for optional dependencies. If the provider is missing, the field is `null` and the plugin stays enabled. Available from version 9.8.
