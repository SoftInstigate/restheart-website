...
**Prerequisites:** 
- link:/docs/1-foundations/introduction[Introduction to RESTHeart]
- link:/docs/1-foundations/core-concepts[Core Concepts]
...
<img src="/images/restheart-security-explained.png" class="img-fluid" alt="RESTHeart security architecture" style="max-width: 600px;" />
</div>
++++
...
Each request passes through multiple security layers:
1. **Authentication Mechanisms** extract credentials from the request (e.g., from headers)
2. **Authenticators** verify those credentials against a user store
3. **Authorizers** check if the verified user has permission for the requested action
...
- **User ID** - Unique identifier (e.g., "admin", "user123")
- **Roles** - List of assigned roles (e.g., ["editor", "viewer"])
- **Custom properties** - Additional data your authenticator provides

This Account is then used by Authorizers to make permission decisions.
...
**Token properties:**
- Stored client-side (your application must save it)
- Default expiration: 15 minutes (configurable)
- Validated without re-checking credentials
...
**Learn more:** link:/docs/4-security/authentication[Authentication Methods]
...
**Learn more:** link:/docs/4-security/authorization[Complete Predicate Reference]
...
**Learn more:** link:/docs/4-security/permissions[Permission Management]
...
2. **Use HTTPS in production** - link:/docs/4-security/tls[TLS Configuration]
3. **Apply least privilege** - Give users minimum required permissions
4. **Use strong passwords** - Enforce password policies
5. **Enable security hardening** - link:/docs/4-security/security-hardening[Security Hardening Guide]
6. **Monitor authentication failures** - link:/docs/5-deployment/auditing[Enable Auditing]
7. **Secure MongoDB connection** - link:/docs/4-security/secure-connection-to-mongodb[MongoDB TLS]
...
* link:/docs/2-mongodb-rest/tutorial[REST API Tutorial] - Use the API with authentication
* link:/docs/4-security/tutorial[Security Tutorial] - Hands-on security configuration
* link:/docs/4-security/user-management[User Management] - Create and manage users
* link:/docs/4-security/permissions[Permissions] - Configure fine-grained access control

=== Security In Depth

* link:/docs/4-security/authentication[Authentication Methods] - All authentication options
* link:/docs/4-security/authorization[Advanced Authorization] - Complex permission rules
* link:/docs/4-security/security-hardening[Security Hardening] - Production security
* link:/docs/7-security-plugins/overview[Security Plugins] - Extend security layer
