...
This guide covers the Sophia Admin Panel — the web-based interface where you manage contexts, knowledge bases, text segments, users, API tokens, chat sessions, costs, errors, and models. Access the admin panel at `/admin` after logging in with an administrator account.
...
A *Context* defines the full behaviour of Sophia for a given knowledge domain: the system prompt template, tag filters, RAG and LLM options, agentic mode settings, collaborative artifacts, hint questions, deck view, and the MCP server description shown to AI clients.
...
Click *New Context* and provide a unique ID (e.g. `support`, `product-docs`). The ID is used in the web URL path (`/{contextId}`) and in the MCP endpoint URL (`/mcp/{contextId}/`).
...
| `_id` | Yes | Context name. Used in URLs and MCP paths.
...
| `private` | No | If `true`, requires a JWT containing this context id in the `contexts` claim. Public contexts are reachable without authentication.
...
Tags are the primary mechanism for knowledge segregation. The context's `tags` are ANDed with every vector search: only documents carrying *all* of the context's tags are retrieved.

- A context with `tags: ["restheart"]` can only access documents tagged `restheart`
- A context with `tags: ["internal", "hr"]` can only access documents tagged with both
...
The context editor exposes these fields directly. The right-hand side shows a model picker with origin (EU/US), pricing per million input/output tokens, and a tool-use score that highlights models suited for agentic workflows.

=== Agentic Mode

Enable *Agentic Mode* on a context to let Sophia operate as an autonomous agent: it iteratively calls tools (search, file retrieval, list-paths, list-tags) before composing a final answer.
...
| `compactSearch` | boolean | `false` | Make `sophia_search` return source/chunk/preview only. ~10× cheaper than full segments.
...
- *Start of turn*: if a saved context exists, the system auto-injects it as a synthetic `sophia_context_load` tool result before the main loop. `historyLimit` is ignored.
- *End of turn*: after the model's text answer, the system runs a separate forced call asking the model to summarise via `sophia_context_save`, `sophia_context_append`, or `sophia_context_skip`. Those tools are not available in the main loop — they are invoked only by the synthetic post-answer step.

This frees the main loop from history-token cost on every turn and keeps long-session coherence. The prompt template should *not* instruct the model to call `sophia_context_*` itself — doing so blocks the synthetic flow.
...
|===
| Option | Type | Default | Description
| `collaborative` | boolean | `false` | Add `render_output` to the agentic loop. After the model's text answer, force a `render_input` call with 2–4 follow-up buttons.
| `renderInputDesc` | string | server default | Override the description of the `render_input` tool. Document domain-specific patterns.
| `renderOutputDesc` | string | server default | Override the description of the `render_output` tool. Document visual style and content guidelines.
|===

`render_output` produces HTML in an iframe injected into the chat — diagrams, calculators, mini-dashboards, REST micro-tools. PicoCSS v2 with dark theme is pre-loaded; the model can also use a `<style>` block for custom CSS.

`render_input` is *not* available in the main loop. It is invoked only by the post-answer synthetic call, which constrains the model to attach 2–4 follow-up buttons. The prompt template should *not* instruct the model to call `render_input` directly — doing so prevents the synthetic flow.

A per-message toggle in the chat UI lets the end user activate Collaborative Mode for a specific question, even on contexts where it is off by default.
...
[source]
----
https://your-sophia-instance/                  → default context
https://your-sophia-instance/{context-id}      → "{context-id}" context
----
...
[source,html]
----
<iframe src="https://your-sophia-instance/{context-id}" ...></iframe>
----
...
. Sessions whose JWT contains this context's id in the `contexts` claim (cookie auth or `Authorization: Bearer`)
...
}' | http -a admin:password PUT :8080/contexts/support
----
...
echo '{"options": {"relevantsLimit": 8, "temperature": 0.2}}' | \
  http -a admin:password PATCH :8080/contexts/support
----

*Delete a context:*
[source,bash]
----
http -a admin:password DELETE :8080/contexts/support
----
...
- *Tags* — free-form, comma-separated. By default the chip "Accessible from public contexts (adds `public` tag)" is on, simulating the most common case; toggle it off to make the upload accessible only to authenticated users matching the context tags.
...
|===
| Role | Capability
| `user` | Default role for end users — can chat against contexts whose id is in the user's `contexts` claim, or any public context.
...
- `contexts` — list of context ids the user is authorized for (relevant for *private contexts*)
...
- *Contexts* — the contexts this token can access. For private contexts, the context id MUST be included here.
...
The list shows recent sessions across all contexts and users, with filters for date range, context, model, and username.
...
- Filtering by context and model
...
Pick a model id from this table when overriding the default in a context.
