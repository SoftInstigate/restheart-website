...
- `inviteToken`, `inviteTokenExpiresAt` — opaque random token and its UTC expiry, generated server-side on invitation. Cleared once the user activates.
...
- *Agents* — the agents the user will be authorised for after activation. Private agents are flagged.
...
The server fills in `status: "invited"`, generates `inviteToken` / `inviteTokenExpiresAt` (via the ACL `mergeRequest` with the `@rnd(bits)` placeholder), and the `RESPONSE_ASYNC` interceptor sends the email. Do *not* include a `password` field — the `UserPersistInterceptor` rejects POSTs that carry one.
