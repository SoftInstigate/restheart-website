...
The Sophia API provides RESTful endpoints for managing the AI service: chat operations, document management, context configuration, user management, API tokens, and supporting services. The API is built on RESTHeart, so most CRUD operations follow standard MongoDB-REST semantics.
...
- `context=<context-id>` — context to use (defaults to `default`)
...
- `collaborative` — boolean, optional. Per-message override that activates Collaborative Mode (`render_output` + post-answer `render_input`) regardless of the context default.

*Request:*
[source,bash]
----
curl -X POST "https://sophia-api.restheart.com/chats?context=support" \
...
When the active context has `agenticMode: true` and `streamThinkingEvents: true`, the stream includes `events[]` entries during the agentic phase:

[cols="1,3", options="header"]
|===
| Event type | Meaning
| `tool_start` | The model invoked a tool (e.g. `sophia_search`). Includes `tool`, `iteration`, `args`.
...
| `render_step` | The model invoked `render_output` to inject an interactive HTML artifact in the chat.
|===
...
Contexts define the behaviour of Sophia for a specific knowledge domain.
...
----
{
  ...
  "options": {
    ...
    "collaborative": false,
    "renderInputDesc": null,
    "renderOutputDesc": null,
    "deckView": false,
    "modelId": null
  },
  "mcp": {
...
*Endpoint:* `PUT /contexts/{contextId}?wm=upsert`

[source,bash]
----
curl -X PUT "https://sophia-api.restheart.com/contexts/support?wm=upsert" \
  -H "Authorization: Bearer <jwt-token>" \
  -H "Content-Type: application/json" \
  -d '{
    "template": "You are Sophia...\n\n<documents-placeholder>",
...
[source,bash]
----
curl -X PATCH "https://sophia-api.restheart.com/contexts/support" \
  -H "Authorization: Bearer <jwt-token>" \
  -H "Content-Type: application/json" \
...
- `context=<context-id>` — applies the context's tag filters

[source,bash]
----
curl "https://sophia-api.restheart.com/textSegments/_aggrs/search?prompt=password%20reset&context=support" \
  -H "Authorization: Bearer <jwt-token>"
----
...
A user can change *only their own password* via this endpoint. Role and context modifications require an admin account.
...
| `403 Forbidden` | Access denied (e.g. private context without correct claim)
