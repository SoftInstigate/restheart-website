...
- *Compact Search*: returns only filename / chunk index / 150-word preview from `sophia_search`, ~10× cheaper than the full segment payload
...
A persistent markdown document keyed by `chatId`, replacing classic history injection. At turn start the saved context is auto-loaded as a synthetic `sophia_context_load` tool result; at turn end a forced synthetic call summarises the turn via `sophia_context_save`, `sophia_context_append`, or `sophia_context_skip`. This frees the main loop from history-token overhead and keeps context coherent across long sessions.
