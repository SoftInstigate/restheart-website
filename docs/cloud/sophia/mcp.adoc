...
Within a context, documents are further organised by *tags*. Pass the optional `tags` parameter to `search` to narrow retrieval to documents carrying specific tags. Use `tags` to discover which tags are available.
...
| `tags` | Lists the tags available in the knowledge base, filtered by the active context and credentials.
...
| `ask` / `show` | Interactive artifacts (forms / visualisations) rendered in an iframe by the client. Exposed only when the context has `collaborative: true` in its options.
|===
