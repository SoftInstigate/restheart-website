...
- `numDimensions: 1024` — matches Amazon Titan Embed Text v2 output dimensions
- `similarity: cosine` — cosine similarity for document matching
- `path: "vector"` — field containing document embeddings

*Filter Fields*

- `path: "metadata.tags"` — enables tag-based access control and knowledge base scoping
- `path: "metadata.pathPrefixes"` — enables path-based filtering (used by `sophia_list_paths` and directory-scoped searches)
