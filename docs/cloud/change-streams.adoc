...
The same editor used for link:aggregations[aggregation pipelines], so the buttons behave the same way:
...
| **Add template**
| Drops in a starting definition, with the `uri` and `stages` already in place.

| **Add pagination**
| Appends `$skip` and `$limit` stages. Rarely what you want on a stream, which delivers events as they happen rather than a page of them.

| **Format**
| Pretty-prints the JSON and validates it. Invalid JSON blocks the save with an inline error.
|===

There is no **Run** button here, and there could not be: a stream is not executed once and finished, it stays open and reports changes as they occur. Use the <<live-try-panel,Live Try panel>> below to watch it instead.

**Variables and optional stages** expands into a reference for `$var` and `$ifvar`. They work in a stream's stages as they do in an aggregation's, with the values coming from the `avars` query parameter on the WebSocket URL — see link:aggregations#avars[Pipeline Variables] for the syntax.
