...
NOTE: `rhc` is available from RESTHeart 9.8.
...
All three are built on link:kit[the Cloud Kit] — sign-up, sign-in, teams and payments — and
configured by link:cli[`rhc`] from an `rhc.setup.ts` committed alongside the code.
