...
[TIP]
====
**New to RESTHeart Cloud?** Start with the link:/docs/cloud/full-stack-example[Full-Stack Web Application Example] — a complete tutorial covering both the backend API and a frontend in Vue.js, React or Angular.
====

[NOTE]
====
**Building rather than reading?** The recipes below are API calls: run them from a terminal, and
they show you what RESTHeart Cloud does. For an application you intend to keep, start from a
starter and skip the typing.

* https://github.com/SoftInstigate/restheart-cloud-starter-react[React^] ·
  https://github.com/SoftInstigate/restheart-cloud-starter-ng[Angular^] — signup, login, OAuth,
  email verification, password reset, invitations and teams, already working
* https://github.com/SoftInstigate/restheart-cloud-starter-ecommerce[Ecommerce (React)^] — the
  above plus a catalog, cart, Stripe Checkout and an order ledger
* https://github.com/SoftInstigate/restheart/tree/master/examples/market-game[The market game^] —
  three AI agents trading through a service's MCP server: one collection, six rules, no code.
  The tutorial for the link:/docs/cloud/mcp[MCP Server]

Each ships an `rhc.setup.ts`, so the collections, indexes and permissions a recipe below creates
by hand are stated as code and applied with one command:
...
See link:/docs/cloud/cli[The `rhc` command line]. The point is not that typing the calls is
wrong — it is that a configuration you will need again should be written down somewhere that runs.
====
...
# Create permission for unauthenticated read access to published articles
...
# Create permission for unauthenticated read access to posts
curl -i -X POST [RESTHEART-URL]/acl \
  -H "Authorization: Basic [BASIC-AUTH]" -H "Content-Type: application/json" \
  -d '{
    "_id": "allCanGetPosts",
...
# Create permission for unauthenticated read access to readings
curl -i -X POST [RESTHEART-URL]/acl \
  -H "Authorization: Basic [BASIC-AUTH]" -H "Content-Type: application/json" \
  -d '{
