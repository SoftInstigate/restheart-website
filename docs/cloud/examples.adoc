...
curl -X PUT https://[INSTANCE-URL]/media -H "Authorization: Basic [BASIC-AUTH]"
...
http PUT [INSTANCE-URL]/media Authorization:"Basic [BASIC-AUTH]"
...
const createContentCollections = () => {
  ...
  fetch('[INSTANCE-URL]/media', { method: 'PUT', headers })
  .then(response => {
    if (response.ok) {
