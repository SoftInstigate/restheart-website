...
curl "[RESTHEART-URL]/articles" \
  -H "Authorization: Basic [BASIC-AUTH]" \
  --data-urlencode "filter={'status':'published'}" --data-urlencode "sort={publishedAt:-1}"

# Get article by slug (requires $unauthenticated read permission or basic auth)
curl "[RESTHEART-URL]/articles" \
  -H "Authorization: Basic [BASIC-AUTH]" \
  --data-urlencode "filter={'slug':'getting-started-restheart-cloud'}"

# Search articles (requires $unauthenticated read permission or basic auth)
curl "[RESTHEART-URL]/articles" \
  -H "Authorization: Basic [BASIC-AUTH]" \
  --data-urlencode "filter={$text:{$search:'RESTHeart API'}}"
----
...
curl "[RESTHEART-URL]/posts" \
  -H "Authorization: Basic [BASIC-AUTH]" \
  --data-urlencode "filter={'author':{$in:['user1','user2','user3']}}" --data-urlencode "sort={createdAt:-1}"

# Search posts by hashtag - requires $unauthenticated read permission or basic auth
curl "[RESTHEART-URL]/posts" \
  -H "Authorization: Basic [BASIC-AUTH]" \
  --data-urlencode "filter={'hashtags':{$in:['#api']}}"

# Get user's posts - requires $unauthenticated read permission or basic auth
curl "[RESTHEART-URL]/posts" \
  -H "Authorization: Basic [BASIC-AUTH]" \
  --data-urlencode "filter={'author':'johndoe'}" --data-urlencode "sort={createdAt:-1}"
----
...
curl -i "[RESTHEART-URL]/readings" \
  -H "Authorization: Basic [BASIC-AUTH]" \
  --data-urlencode "filter={'timestamp':{$gte:'2024-01-15T00:00:00Z'}}" --data-urlencode "sort={timestamp:-1}"
