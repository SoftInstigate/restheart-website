...
Now that you have a working backend, check out our link:#frontend-examples[Frontend Examples] section to build a complete web application with Vue.js, React, or Angular!

=== Frontend Examples

Ready to build complete web applications with Vue.js, React, or Angular? We've created three full-featured frontend implementations that demonstrate how to build production-ready applications with RESTHeart Cloud.
...
Before you start with the frontend examples, make sure you have completed the backend setup from the link:#e-commerce-product-catalog[E-commerce Product Catalog] example, including:

* Creating the `products` and `categories` collections
* Adding sample product data
* Configuring ACL for unauthenticated read access to products
...
* Add more features like cart functionality, user authentication, or reviews
* Extend the backend with additional collections and relationships
* Deploy your application to production

For more backend examples and use cases, continue exploring the sections below.
...
curl -i -X POST [RESTHEART-URL]/acl \
  -H "Authorization: Basic [BASIC-AUTH]" -H "Content-Type: application/json" \
  -d '{
    "_id": "allCanGetPublishedArticles",
    "roles": ["$unauthenticated"],
...
curl -i -X POST https://[instance].restheart.com/acl \
  -H "Authorization: Bearer [token]" -H "Content-Type: application/json" \
  -d '{
        "_id": "allCanGetPublicDatasets",
        "roles": ["$unauthenticated"],
...
curl -i -X POST https://[instance].restheart.com/acl \
  -H "Authorization: Bearer [token]" -H "Content-Type: application/json" \
  -d '{
        "_id": "allCanGetPublicEventsLimitedFields",
        "roles": ["$unauthenticated"],
