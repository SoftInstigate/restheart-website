---
docs_version: 9
title: RESTHeart Cloud
layout: docs-adoc
menu: cloud
applies_to: cloud
---

=== Introduction

RESTHeart Cloud is a fully managed Platform as a Service (PaaS) that revolutionizes backend development by providing **instant, production-ready APIs in minutes**. Built on RESTHeart's proven framework, it covers 80% of modern application backend needs out-of-the-box while offering a powerful low-code framework for the remaining 20%.

Whether you're building web applications, mobile apps, IoT solutions, or data analytics platforms, RESTHeart Cloud eliminates the complexity of backend infrastructure, letting you focus on what makes your application unique.

=== Why RESTHeart Cloud?

==== Get Your Backend in Minutes

* **Zero Infrastructure Setup**: No servers, databases, or deployment pipelines to configure
* **Instant API Creation**: Generate REST, GraphQL, and WebSocket APIs with a few clicks
* **Pre-built Authentication**: User management and security ready out-of-the-box
* **Extend the API**: Add custom plugins or use marketplace extensions

==== Perfect for Rapid Development

RESTHeart Cloud accelerates development for:

* **Web Applications**: Full-stack applications with authentication, data management, and real-time features
* **Mobile Backends**: REST and GraphQL APIs for iOS, Android, and cross-platform apps
* **IoT Projects**: Collect, store, and analyze sensor data with real-time dashboards
* **Data Analytics**: Aggregation pipelines, reporting APIs, and data visualization backends
* **Prototyping**: Validate ideas quickly with fully functional APIs
* **Headless CMS**: Content management with multi-channel delivery

=== Key Features

==== Instant API Creation
Spin up fully functional API endpoints in seconds with:

* MongoDB integration for data persistence
* Automatic CRUD operations
* Built-in query and filtering capabilities
* Real-time data streams

==== Multiple API Types
* **REST API**: Complete MongoDB operations via HTTP
* **GraphQL API**: Type-safe queries with automatic schema generation
* **WebSocket API**: Real-time updates using MongoDB Change Streams

==== Built-in Security
* JWT-based authentication
* Role-based access control (RBAC)
* User management via `/users` collection
* Permission management via `/acl` collection
* HTTPS encryption by default
* Automatic credentials encryption
* Automatic protection against brute force and dictionary attacks

==== Flexible Deployment Options
Choose the right tier for your needs:

[cols="1,3a"]
|===
| Tier | Features

| **Free**
| * Perfect for prototyping and learning
* MongoDB included and managed by RESTHeart Cloud
* Limited storage and request volume
* Shared infrastructure

| **Shared**
| * Ideal for small to medium applications
* MongoDB included and managed by RESTHeart Cloud
* Higher request limits and more storage
* Support
* Basic monitoring and analytics
* Plugins from marketplace

| **Dedicated**
| * Production-ready for enterprise applications
* **Requires your own MongoDB** (e.g. MongoDB Atlas, on-premise) — not included
* Dedicated resources and isolated infrastructure
* Custom domain support
* Priority support with SLA
* Advanced monitoring, logging, and analytics
* Custom plugins and extensions
|===

=== Security

* All communication encrypted via HTTPS/TLS 1.3
* JWT-based authentication with configurable expiration
* Role-based access control with fine-grained permissions
* Automatic security updates and patches

=== Pricing

For detailed pricing information and feature comparisons, visit https://cloud.restheart.com/pricing

=== Cloud UI — Management Console

The RESTHeart Cloud UI is a web-based management console that lets you manage every aspect of your service instance without writing a single API call. The following pages document each feature area:

[cols="1,3"]
|===
| Page | Description

| link:ui-overview[**UI Overview**]
| Introduction to the management console, service tier differences, Admin JWT, and cross-cutting UI behaviours (search, pagination, JSON editing, error handling).

| link:managing-data[**Collections & Documents**]
| Browse, create, edit, and delete collections and documents. GridFS bucket detection. Assign JSON Schema validators from the Collection Browser.

| link:managing-users[**User Management**]
| Create and manage the users that authenticate against your service via Basic Auth or Token Auth. Password hashing, roles, search, and pagination.

| link:managing-permissions[**Permissions (ACL)**]
| Define role-based access control rules using Undertow predicates. Configure row-level security, field masking, and request merging via `mongo` options.

| link:managing-indexes[**Managing Indexes**]
| List, create, and delete MongoDB indexes on any collection. Single-field, compound, unique, sparse, TTL, and full-text index types. Improve query and sort performance without writing API calls.

| link:aggregations[**Aggregation Pipelines**]
| Define server-side MongoDB aggregation pipelines, run them from the editor before saving, expose them as REST endpoints, and use pipeline variables via `$var` / `avars`.

| link:constraints[**Data Constraints**]
| Rules the whole collection must satisfy, checked in a transaction on every write and rolled back when broken.

| link:mcp[**MCP Server**]
| Publish selected collections, aggregations, change streams and GraphQL apps to AI agents over the Model Context Protocol, and see the catalogue as one of your users would.

| link:webhooks[**Webhooks**]
| Fire HTTP callbacks or send emails in response to API events. Enable/disable the webhooks plugin, manage HTTP and email webhook types, and view execution logs.

| link:graphql[**GraphQL Applications**]
| Create and manage GraphQL app definitions that map MongoDB collections to a GraphQL schema. Visual Editor and JSON Editor modes. Enable/disable individual apps.

| link:schemas[**JSON Schema Validation**]
| Manage reusable JSON Schema documents (draft-07) stored in `/_schemas`. Assign schemas to collections as document validators from the Collection Browser.

| link:change-streams[**WebSocket Change Streams**]
| Define server-side MongoDB Change Stream endpoints exposed as WebSocket connections. Built-in Live Try panel, authentication via Basic Auth in the WebSocket URL.

| link:plans[**Dedicated vs. Free/Shared Plans**]
| Side-by-side comparison of plan differences: database selector, system collection paths, and UI behaviour variations by tier.

| link:custom-mongodb[**Connecting Your MongoDB (Dedicated)**]
| Step-by-step guide for supplying your own MongoDB connection string when provisioning a Dedicated service.

| link:tokens[**Tokens**]
| Personal access tokens for the `rhc` CLI and CI, and short-lived service admin tokens for calling one service by hand. What each reaches, and what neither can do.

| link:stripe[**Stripe Billing**]
| Put the `stripe` plugin on your service: subscriptions or a product catalog, your Stripe credentials, and the permissions a guest checkout needs.
|===

=== Building the App

link:kit[**The Cloud Kit**] is the client half of RESTHeart Cloud: sign-up, sign-in, email verification, password reset, teams and invitations, and — for an app that sells something — subscriptions, seat licences and orders. A framework-agnostic core in plain TypeScript with no dependencies, plus adapters for Angular, React and Vue, and server-side support for Next.js and Nuxt.

=== Automation

The console is the right tool for looking around. For configuration you need twice, link:cli[**the `rhc` command line**] applies a file committed to git — collections, indexes, permissions, plugins and their settings — the same way from a terminal or a CI pipeline. Every step is a check and an apply, so a run against a service already configured writes nothing.

=== Next Steps

Ready to build your backend in minutes?

* link:getting-started[Getting Started Guide] - Create your first API
* link:ui-overview[Cloud UI Overview] - Explore the management console
* link:users-and-permissions[Users and Permissions] - Set up authentication and authorization
* link:kit[The Cloud Kit] - Add sign-up, sign-in and payments to your frontend
* link:cli[The `rhc` CLI] - Configure a service from a file in git
* link:tokens[Tokens] - The credentials for automation, and for calling a service by hand
* link:security[Security Best Practices] - Secure your applications
* https://cloud.restheart.com[Sign Up Now] - Start building today
