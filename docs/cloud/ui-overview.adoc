...
The Cloud UI covers these integrated feature areas:
...
| link:guards[**Guards**]
| Block or redirect requests that match your own rules — unaccepted terms, a lapsed subscription, feature gating.

| link:origin-allowlist[**Origin Allowlist**]
| Restrict which websites can call your API, as protection against CSRF from other sites.
...
| Refuse writes that would break a rule about the collection as a whole — no negative balance, no double booking.

| link:mcp[**MCP Server**]
| Publish collections, aggregations, change streams and GraphQL apps to AI agents over the Model Context Protocol.
...
* You do **not** need to manage this token yourself for anything the console does — it handles the lifecycle, including renewal when it expires.

You can also ask for one deliberately, when you want to call the service by hand from a terminal: the service's **Connect** page hands you one alongside a ready-made `curl` line, and the link:tokens[Tokens] page does too if you would rather pick the service from a list. It lasts fifteen minutes either way.

For automation, this is the wrong credential — fifteen minutes is not a pipeline. That is a link:tokens#personal-access-tokens[personal access token], which the link:cli[`rhc` CLI] authenticates with.
