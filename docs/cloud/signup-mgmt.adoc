...
Once Sign-up Management is enabled on a service, the generic `/users` REST resource is automatically restricted for that service's own end users, to enforce security: creating or replacing a user document (`POST`/`PUT`) is always rejected, and a `PATCH` is rejected if it touches `roles`, `team`/`teams`, `password`, or a handful of other account-management fields — regardless of what ACL rules the service defines. Every other field, including `profile.*` and application-level data such as `consents`, is left entirely to the service's own ACL.

This prevents a user who is only granted permission to edit their own profile (or other self-service data) from also being able to rewrite their own `roles` or team membership — e.g. self-promoting to `owner` of a team, or joining a team they were never invited to.
...
== OAuth — Google Social Login [[oauth]]
