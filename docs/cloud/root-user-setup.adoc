...
[source,bash]
----
curl -i -X POST [INSTANCE-URL]/users?wm=upsert \
...
[source,bash]
----
http POST [INSTANCE-URL]/users?wm=upsert \
...
[source,javascript]
----
fetch('[INSTANCE-URL]/users?wm=upsert', {
  method: 'POST',
...
[source,bash]
----
curl -i -X POST [INSTANCE-URL]/acl?wm=upsert \
  -H "Authorization: Bearer [JWT]" \
  -H "Content-Type: application/json" \
...
[source,bash]
----
http POST [INSTANCE-URL]/acl?wm=upsert \
  Authorization:"Bearer [JWT]" \
...
[source,javascript]
----
fetch('[INSTANCE-URL]/acl?wm=upsert', {
  method: 'POST',
