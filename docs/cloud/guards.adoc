...
Add a JSON Schema on `/users` (see link:schemas[JSON Schema Validation]) making `consents` mandatory, so a user document cannot exist without it. This applies to sign-ups through `/auth/register` too.
