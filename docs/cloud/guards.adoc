...
| `@roles`
| The caller's roles, as an array — `["$unauthenticated"]` when nobody is signed in. See <<Rules that should not apply to anonymous callers>>.
...
NOTE: `@roles` is available from RESTHeart 9.8. On earlier versions, naming the paths is the only
option.
