...
NOTE: These count against your service's link:emails[daily email allowance], and a webhook with
several recipients takes them all or none — five addresses with three left sends nothing. Set up
your own mail server and no limit applies.
...
----
Condition: `method('POST') or method('PUT') or method('PATCH')`
