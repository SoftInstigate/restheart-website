...
NOTE: If your service also has link:signup-mgmt[Sign-up Management] enabled, the `/users` resource additionally enforces a restriction limiting your own end users to self-service `profile.*` edits (RESTHeart 9.6.0+). This User Management UI is unaffected and continues to create, edit (including `roles`), and delete users normally — RESTHeart Cloud manages your service on your behalf using a role that's exempt from that restriction. See link:signup-mgmt#users-self-service-write-restriction[Sign-up Management] for details.
